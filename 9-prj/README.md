...
<h4>Подготовь систему bp 2-х VMs как в 7 разделе (который про сеть), а после этого запусти `sudo bash 9-prj/task9-prj.sh`</h4>
...
<h4>Теперь приложение должно запуститься. Ты можешь получить доступ к нему введя в браузере `http://localhost:30080`.</h4> 
